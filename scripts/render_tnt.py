...
def post_process_mesh(mesh, cluster_to_keep=1, cluster_keep_range=None):
    """
    Post-process a mesh to filter out floaters and disconnected parts
    """
    if cluster_keep_range is not None:
        print(f"post processing the mesh to keep cluster ranks in range {cluster_keep_range}")
    else:
    ...
    if cluster_keep_range is not None:
        if len(cluster_keep_range) == 1:
            start_rank = end_rank = int(cluster_keep_range[0])
        elif len(cluster_keep_range) == 2:
            start_rank, end_rank = sorted(int(v) for v in cluster_keep_range)
        else:
            raise ValueError(
                f"cluster_keep_range must contain 1 or 2 values, got {cluster_keep_range}"
            )
        if start_rank < 1:
            raise ValueError(f"cluster_keep_range is 1-based and must start from 1, got {cluster_keep_range}")
        sorted_cluster_ids = np.argsort(cluster_n_triangles)[::-1]
        keep_cluster_ids = sorted_cluster_ids[start_rank - 1 : end_rank]
        if keep_cluster_ids.size == 0:
            raise ValueError(
                f"cluster_keep_range {cluster_keep_range} is outside the available cluster count {len(sorted_cluster_ids)}"
            )
        triangles_to_remove = ~np.isin(triangle_clusters, keep_cluster_ids)
    else:
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            ...
            mesh = post_process_mesh(mesh, num_cluster, cluster_keep_range=cluster_keep_range)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional 1-based cluster rank(s) to keep: one value keeps that single rank, two values keep the inclusive rank range, e.g. 2 or 2 3.",
    )
    ...
    render_sets(
        ...
        cluster_keep_range=getattr(args, "cluster_keep_range", None),
    )